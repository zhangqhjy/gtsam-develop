...
namespace gtsam {
   ...
   * 非线性因子图是非高斯因子图，即非线性因子，其源自NonlinearFactor。 值结构通常（在SAM中）不仅仅是矢量，例如Rot3或Pose3，
   * 它们是非线性流形中的对象。 线性化非线性因子图在线性化点处的切向量空间上创建线性因子图。 因为切线空间是真正的向量空间，
   * 所以配置类型将是该线性化因子图中的VectorValues。
   */
