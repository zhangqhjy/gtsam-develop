...
namespace gtsam {
  ...
  * localCoordinates.
   * 包含任何类型的Manifold-group元素的非模板化配置。 值结构是从key到值的映射。
   * 它用于指定因子图中一组变量的值。 值是一个值结构，它可以保存作为流形上元素的变量，而不仅仅是向量。
   * 然后，它作为一个整体实现了一个聚合类型，它也是一个流形元素，因此支持操作dim，retract和localCoordinates。
  */
